...
export const ChatHeader = ({
  ...
  return (
    <div className="sticky top-0 z-10 p-4 sm:p-5 border-b border-b-border/40 bg-white dark:bg-gray-800 flex items-center gap-3 shadow-sm">
      {onBackClick && (
        <Button variant="ghost" size="icon" onClick={onBackClick} className="flex-shrink-0 hover:bg-muted/50">
          <ArrowLeft className="h-5 w-5" />
          <span className="sr-only">{t('messages.back_to_list')}</span>
        </Button>
      )}
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs sm:text-sm text-gray-500 truncate">
          {chat.type === "group" ? 
            `${chat.members.length} ${t('messages.members')}` : 
            t('messages.private_conversation')}
        </p>
      </div>
    </div>
  );
